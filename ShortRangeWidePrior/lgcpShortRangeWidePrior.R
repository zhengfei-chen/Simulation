...
#     Short prior
# Warnings 
# When there is evidence in the data for quite large values of the spatial correlation parameter
# One possible way of rectifying this issue is to restart the chain using a larger value of 'ext' 
# in the call to lgcpPredictSpatialPlusPars. 
# You should do this if the warning message is repeated many times. 
# If this warning message appears at all then you should be warned 
# that inference from this run may not be reliable: the proposed move at this iteration was rejected.
...
eventsSample5k = myLgcp$events[
  sample(length(myLgcp$events), 2500)]
#'
...
priors <- lgcpPrior(
  etaprior=PriorSpec(LogGaussianPrior(
    mean=log(c(0.5, 10000/2)),
    variance=diag(c(0.1, 0.4)))),
